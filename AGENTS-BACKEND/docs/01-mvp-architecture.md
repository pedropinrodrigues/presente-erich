...
O MVP usa workflows explícitos no próprio código. LangGraph ou LangChain não são dependências iniciais: só serão introduzidos se o fluxo precisar de estado, ramificações ou observabilidade que a orquestração simples não resolva bem. Redis, fila dedicada, grafo especializado, microsserviços e deploy da aplicação também são evoluções posteriores.
